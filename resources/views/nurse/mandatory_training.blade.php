...
<script>
    ...
    if ($(".man_ed").val() != "") {
      ...
      console.log("org_country1",org_country1.length);
      ...
      for(var i=0;i<org_country1.length;i++){
        console.log("suborg_country",org_country1.length);
                        ...
                        $(".core_man_con_data_div-"+id).append('<div class="core_man_div core_man_'+data1.courses_id+'">\
                          ...
                          <div class="core_man_div row core_man_institution">\
                            ...
                            <div class="form-group col-md-12">\
                              ...
                              <input class="form-control coreman_upload_certification core_man_imgs_'+data1.courses_id+' coreman_upload_certification-'+data1.courses_id+'" type="file" name="" onchange="changeEvidenceImg(\''+user_id+'\',\''+id+'\',\''+data1.courses_id+'\',\''+well_sal_data+'\'" multiple>\
                            </div>\
                          </div>\
                        </div>');
    ...
    function changeEvidenceImg(user_id,lang_id,language_id,name_arr) {
      ...
      if (!selectedEvidenceFiles[language_id]) {
        selectedEvidenceFiles[language_id] = [];
      }
      
      const newFiles = Array.from($('.upload_evidence-'+language_id)[0].files);

      newFiles.forEach(file => {
        const exists = selectedEvidenceFiles[language_id].some(f => f.name === file.name && f.lastModified === file.lastModified);
        if (!exists) {
            selectedEvidenceFiles[language_id].push(file);
        }
      });

      

        const count = selectedEvidenceFiles[language_id].length;
          console.log("evidence_count", count);
    
      var form_data = "";
      form_data = new FormData();

      for (var i = 0; i < selectedEvidenceFiles[language_id].length; i++) {
        form_data.append(name_arr+"["+language_id+"][]", selectedEvidenceFiles[language_id][i], selectedEvidenceFiles[language_id][i]['name']);
      }
