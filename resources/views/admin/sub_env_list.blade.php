...
@section('content')
<x-card-component parentHeading="Work Preferences & Flexibility Management" childHeading="Work Environment & Preferences" parentUrl="{{route('admin.dashboard')}}" />
            ...
            <div class="table-responsive rounded-2 mb-4">
                <table  class="table border table-striped table-bordered text-nowrap">
                    ...
                    <tbody>
                        @php $i=1 @endphp
                        @if ($enviromentData)
                            @foreach ($enviromentData as $key => $item)
                                <tr>
                                    <td>{{ $i }}</td>
                                    <td>
                                        <div class="">
                                            <span class="mb-0 fw-normal fs-3">{{ $item->env_name }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-1">
                                            
                                            <a href="{{ route('admin.subsub_env_list', ['id' => $item->sub_env_id,'sub_env_id' => $item->prefer_id]) }}"
                                                class="btn btn-primary" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                title="View">
                                                View
                                            </a>
                                            <button href="javascript:void(0)" class="btn btn-success"  onclick="return getCountry({{ $item->prefer_id }})">
                                                Edit
                                            </button>
                                            <button type="button" onclick="return deleteCountry({{ $item->prefer_id }})"
                ...
                <form id="AddCountry"  onsubmit="return addCountry()">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="sub_env_id" value="{{ $prefer_id }}">
                ...
                success: function(res) {
                    ...
                    if (res.status == '2') {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = '{{ route("admin.sub_env_list", ["id" => $prefer_id]) }}';
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
